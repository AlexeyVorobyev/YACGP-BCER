...
import ru.yacgp.bcer.exceptions.MapperListException
import java.util.*
...
object JsonMapper {
     ...
     */
    fun asUUID(value: JsonNode): UUID = UUID.fromString(value.asText())

    /**
     * Преобразование текстового значения в UUID или null.
     *
     * @return UUID идентификатор или null, если значение само null или содержит его
     */
    fun asUUIDOrNull(value: JsonNode?): UUID? =
        value?.asText(null)?.takeIf { it != "null" }?.let { UUID.fromString(it) }
}
